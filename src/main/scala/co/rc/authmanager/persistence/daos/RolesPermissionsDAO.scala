 ...
 */
class RolesPermissionsDAO( val profile: JdbcProfile ) extends EntityActions( profile )
  ...
  override def idLens: Lens[ RolePermission, Option[ Int ] ] = lens { element: RolePermission => element.id } { ( element, id ) => element.copy( id = id ) }
  override def $id( table: RolesPermissions ): jdbcProfile.api.Rep[ Int ] = table.id
  override def tableQuery: jdbcProfile.api.TableQuery[ RolesPermissions ] = RolesPermissions
