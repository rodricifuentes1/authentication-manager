 ...
 */
class IdentificationTypesDAO( val profile: JdbcProfile ) extends EntityActions( profile )
  ...
  override def idLens: Lens[ IdentificationType, Option[ Int ] ] = lens { element: IdentificationType => element.id } { ( element, id ) => element.copy( id = id ) }
  override def $id( table: IdentificationTypes ): jdbcProfile.api.Rep[ Int ] = table.id
  override def tableQuery: jdbcProfile.api.TableQuery[ IdentificationTypes ] = IdentificationTypes
